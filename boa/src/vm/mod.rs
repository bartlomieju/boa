...
pub use code_block::{CodeBlock, ThisMode};
use gc::Gc;
...
impl Context {
    fn execute_instruction(&mut self) -> Result<bool> {
        ...
        match opcode {
            ...
            Opcode::PushUndefined => self.vm.push(Value::undefined()),
            Opcode::PushNull => self.vm.push(Value::null()),
            ...
            Opcode::PushNaN => self.vm.push(Value::nan()),
            Opcode::PushPositiveInfinity => self.vm.push(Value::positive_inifnity()),
            Opcode::PushNegativeInfinity => self.vm.push(Value::negative_inifnity()),
            Opcode::PushLiteral => {
                let index = self.vm.read::<u32>() as usize;
                let value = self.vm.frame().code.literals[index].clone();
                self.vm.push(value)
            }
            Opcode::PushEmptyObject => self.vm.push(Value::new_object(self)),
            Opcode::PushNewArray => {
            ...
            Opcode::In => {
                ...
                self.vm.push(self.has_property(&rhs, &key));
            }
            Opcode::InstanceOf => {
                let y = self.vm.pop();
                let x = self.vm.pop();
            ...
            Opcode::Void => {
                let _ = self.vm.pop();
                self.vm.push(Value::undefined());
            }
            Opcode::TypeOf => {
                let value = self.vm.pop();
                self.vm.push(value.get_type().as_str());
            }
            Opcode::Pos => {
                let value = self.vm.pop();
            ...
            Opcode::DefVar => {
                ...
                self.create_mutable_binding(name.to_string(), false, VariableScope::Function)?;
            }
            Opcode::DefLet => {
                let index = self.vm.read::<u32>();
                let name = self.vm.frame().code.variables[index as usize].clone();

                self.create_mutable_binding(name.to_string(), false, VariableScope::Block)?;
            }
            Opcode::DefConst => {
                let index = self.vm.read::<u32>();
                let name = self.vm.frame().code.variables[index as usize].clone();

                self.create_immutable_binding(name.to_string(), false, VariableScope::Block)?;
            }
            Opcode::InitLexical => {
                let index = self.vm.read::<u32>();
                let value = self.vm.pop();
            ...
            Opcode::SetName => {
                ...
                if self.has_binding(&name) {
                    ...
                    self.set_mutable_binding(&name, value, true)?;
                } else {
                    self.create_mutable_binding(name.to_string(), true, VariableScope::Function)?;
                    self.initialize_binding(&name, value)?;
                }
            }
            Opcode::Jump => {
            ...
            Opcode::Call => {
                ...
                let object = match func {
                    Value::Object(ref object) if object.is_callable() => object.clone(),
